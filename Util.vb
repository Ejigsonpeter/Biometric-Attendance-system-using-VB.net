...
Public Class Util
    ...
    (ByVal hwnd As Int32, ByVal hdc As Int32) As Int32
    ...
    Public Sub New(ByRef lbLog As ListBox, ByVal pbPic As PictureBox, _
    ...
    Public Sub WriteError(ByVal errorCode As Integer)
        Select Case errorCode
            ...
            Case GRConstants.GR_ERROR_NOT_INITIALIZED
                WriteLog("The GrFingerX Library is not initialized. (Error:" & _
                errorCode & ")")
            Case GRConstants.GR_ERROR_FAIL_LICENSE_READ
                WriteLog( _
                "License not found. See manual for troubleshooting. (Error:" _
                & errorCode & ")")
                MessageBox.Show( _
                "License not found. See manual for troubleshooting.")
            Case GRConstants.GR_ERROR_NO_VALID_LICENSE
                WriteLog( _
                "The license is not valid. See manual for " & _
                "troubleshooting. (Error:" & errorCode & ")")
                MessageBox.Show( _
                "The license is not valid. See manual for troubleshooting.")
            Case GRConstants.GR_ERROR_NULL_ARGUMENT
                WriteLog("The parameter have a null value. (Error:" & _
                errorCode & ")")
            Case GRConstants.GR_ERROR_FAIL
                WriteLog("Fail to create a GDI object. (Error:" & errorCode & ")")
            Case GRConstants.GR_ERROR_ALLOC
                WriteLog("Fail to create a context. Cannot " & _
                "allocate memory. (Error:" & errorCode & ")")
            Case GRConstants.GR_ERROR_PARAMETERS
                WriteLog("One or more parameters are out of " & _
                "bound. (Error:" & errorCode & ")")
            Case GRConstants.GR_ERROR_WRONG_USE
                WriteLog("This function cannot be called at " & _
                "this time. (Error:" & errorCode & ")")
            ...
            Case GRConstants.GR_ERROR_SIZE_OFF_RANGE
                WriteLog("Image is too larger or too short. (Error:" & _
                errorCode & ")")
            Case GRConstants.GR_ERROR_RES_OFF_RANGE
                WriteLog("Image have too low or too high resolution. (Error:" & _
                errorCode & ")")
            Case GRConstants.GR_ERROR_CONTEXT_NOT_CREATED
                WriteLog("The Context could not be created. (Error:" & _
                errorCode & ")")
            ...
            Case GRConstants.GR_ERROR_CONNECT_SENSOR
                WriteLog("Error while connection to sensor. (Error:" & _
                errorCode & ")")
            Case GRConstants.GR_ERROR_CAPTURING
                WriteLog("Error while capturing from sensor. (Error:" & _
                errorCode & ")")
            Case GRConstants.GR_ERROR_CANCEL_CAPTURING
                WriteLog("Error while stop capturing from sensor. (Error:" & _
                errorCode & ")")
            ...
            Case GRConstants.GR_ERROR_INVALID_EXT
                WriteLog("The File have a unknown extension. (Error:" & _
                errorCode & ")")
    ...
    Public Sub PrintBiometricDisplay( _
    ByVal biometricDisplay As Boolean, ByVal context As Integer)
        ...
        If biometricDisplay Then
            ...
            _GrFingerX.BiometricDisplay( _
            template.tpt, raw.img, raw.width, raw.height, _
            raw.res, hdc, handle, context)
        Else
            ' get raw image
            _GrFingerX.CapRawImageToHandle( _
            raw.img, raw.width, raw.height, hdc, handle)
        End If
    ...
    Function ExtractTemplate() As Integer
        ...
        ret = _GrFingerX.Extract( _
raw.img, raw.width, raw.height, raw.res, template.tpt, template.Size, _
GRConstants.GR_DEFAULT_CONTEXT)
    ...
    Public Function Identify(ByRef score As Integer) As Integer
        ...
        For i = 1 To templates.Length
            ...
            If Not (templates(i - 1).template Is Nothing) Then
                Dim tempTpt As Array = _
                Array.CreateInstance(GetType(Byte), _
                templates(i - 1).template.Size)
                Array.Copy(templates(i - 1).template.tpt, tempTpt, _
                templates(i - 1).template.Size)
                ret = _GrFingerX.Identify(tempTpt, score, _
                GRConstants.GR_DEFAULT_CONTEXT)
            End If
    ...
    Public Function Verify(ByVal id As Integer, ByRef score As Integer) As Integer
        ...
        Return _GrFingerX.Verify(tempTpt, tptref, score, _
        GRConstants.GR_DEFAULT_CONTEXT)
    End Function
